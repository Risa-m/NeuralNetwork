...
class Graph:
   ...
   # x is 10000 * 784 vector
    ...
    def graph(self):
        ...
        #    deltaLeru = backOfReLU(inputX2, deltaX2)
            _, deltaW1, deltaB1 = backOfConnect(inputX1, w1, deltaSig)

            h1 += deltaW1 * deltaW1
            w1 -= ETA / np.sqrt(h1) * deltaW1
            b1 -= (deltaB1 * ETA)
            h2 += deltaW2 * deltaW2
            w2 -= ETA / np.sqrt(h2) * deltaW2
            b2 -= (deltaB2 * ETA)
        ...
        plt.show()
        #np.savez('learningMSGD.npz', w1=w1, b1=b1, w2=w2, b2=b2)
