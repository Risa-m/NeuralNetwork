...
class RMSProp(le.Learning):
    ...
    def renewParam(self, deltaW1, deltaB1, deltaW2, deltaB2):
        self.w1 -= self.renewRMSP(self.h1, deltaW1)
        self.w2 -= self.renewRMSP(self.h2, deltaW2)
        """
        self.h1 = RHO * self.h1 + (1.0 - RHO) * deltaW1 * deltaW1
        self.w1 -= ETA / (np.sqrt(self.h1) + EPSILON) * deltaW1
        self.b1 -= (deltaB1 * ETA)
        self.h2 = RHO * self.h2 + (1.0 - RHO) * deltaW2 * deltaW2
        self.w2 -= ETA / (np.sqrt(self.h2) + EPSILON) * deltaW2
        self.b2 -= (deltaB2 * ETA)
        """

    def renewRMSP(self, h, deltaW):
        h = RHO * h + (1.0 - RHO) * deltaW * deltaW
        return ETA / (np.sqrt(h) + EPSILON) * deltaW
